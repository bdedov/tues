...
class DevServer {
     ...
     */
    constructor(proxy = _1.BUILD_DEFAULTS.DEFAULT_PROXY, port = _1.BUILD_DEFAULTS.DEFAULT_PORT, ssl = false, localDomain = 'localhost', customProxyPaths = '^(\/apps\/public\/).*$') {
        this.port = port;
        this.ssl = ssl;
    ...
    startDevServer(apps, staticFolders = []) {
        console.info(`Proxying requests to remote instance ${this.config.proxy.target}`);
        this.apps = apps;
        ...
        this.staticFolders.forEach(staticFolder => {
            ...
            const url = `/${_1.BUILD_DEFAULTS.DEFAULT_PUBLIC_PATH}/${staticFolderContextPath}`;
            this.expressApp.use(url, express.static(staticFolder.path));
        });
        // set custom proxy path
        if (this.customProxyPaths !== true && this.customProxyPaths) {
            const addationalProxyRegex = new RegExp(this.customProxyPaths, 'i');
            this.expressApp.use(addationalProxyRegex, proxyServer(this.config.proxy));
        }
    ...
    getStaticFoldersStatusLog() {
        return this.staticFolders.map(staticFolder => `${this.config.publicPath}${this.getStaticFolderContextPath(staticFolder)}/`).join('\n');
    }
